...
def compute_hype_and_wilson_score_deviation(app_id):
    ...
    wilson_score_deviation = get_wilson_score_deviation(
        review_dict,
    ...
    sentence = (
        ...
        + ': {0} ({1} joke ; {2} acceptable)'
    )
    print(
        sentence.format(
            num_reviews,
            num_reviews_joke_only,
            num_reviews_acceptable_only,
        ),
    )
...
def print_ranking_according_to_keyword(hype_dict, keyword='hype'):
    ...
    hype_ranking = sorted(
        hype_dict.keys(),
        key=lambda x: hype_dict[x][keyword],
        reverse=True,
    )
...
def main():
    ...
    for appID in app_id_list:
        ...
        result_dict[appID][
            'proportion_confirmed-English-tags'
        ] = prct_confirmed_English_tags_among_all_tags
