...
def get_review_sentiment_dictionary(
    app_id,
        ...
        if review['language'] in accepted_languages:
            ...
            if not (detected_language in accepted_languages_iso):
                ...
                if verbose_reviews_wrongly_tagged_as_written_in_english:
                    print(
                        '\nReview #'
                        + str(count_reviews_wrongly_tagged_as_written_in_english)
    ...
    review_dict['language_tag'][
        'num_reviews_wrongly_tagged_English'
    ] = count_reviews_wrongly_tagged_as_written_in_english
    review_dict['language_tag'][
    ...
    sentence = (
        'Number of reviews: {0} ({1} with English tag ; {2} with another language tag).'
    )
    print(
        sentence.format(
            review_dict['language_tag']['num_reviews'],
    ...
    review_dict['language_tag'][
        'prct_confirmed_English_tags_among_English_tags'
    ] = percentage_confirmed_english_tags
    # noinspection PyPep8
    review_dict['language_tag']['prct_confirmed_English_tags_among_all_tags'] = max(
        0,
        percentage_confirmed_english_tags,
    ) * max(0, percentage_reviews_tagged_as_written_in_english)

    accepted_languages_as_concatenated_str = ' '.join(
        l.capitalize() for l in accepted_languages
    )
    sentence = (
        'Number of reviews tagged as in '
        + accepted_languages_as_concatenated_str
...
def classify_reviews(review_dict, sentiment_threshold=None, verbose=False):
    ...
    for keyword in ['positive', 'negative']:
        ...
        # noinspection PyPep8
        acceptable_review_ids_wrt_polarity = [
            reviewID
            for reviewID in current_review_ids
            if bool(
                review_dict[keyword][reviewID]['polarity']
                < sentiment_threshold['polarity'][0]
                or review_dict[keyword][reviewID]['polarity']
        ...
        acceptable_review_ids_wrt_subjectivity = [
            reviewID
            for reviewID in current_review_ids
            if bool(
                sentiment_threshold['subjectivity'][0]
                <= review_dict[keyword][reviewID]['subjectivity']
...
def get_dictionary_wilson_score(review_dict, verbose=False):
    ...
    if verbose:
        ...
        if num_reviews > 0:
            sentence = (
                'Number of reviews: {0} ({1} up ; {2} down) ; Wilson score: {3:.2f}'
            )
...
def main(argv):
    ...
    review_dict = get_review_sentiment_dictionary(
        app_id,
        accepted_languages,
    ...
    (acceptable_reviews_dict, joke_reviews_dict) = classify_reviews(
        review_dict,
    ...
    print(
        '\nStats for all reviews available in '
        + ' '.join([l.capitalize() for l in accepted_languages]),
    )
    ...
    wilson_score_acceptable_only = get_dictionary_wilson_score(
